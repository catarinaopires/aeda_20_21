...
public:
	...
	{
        CNode *currentL1 = first; CNode *currentL2 = lst.first;
        while(currentL2 != 0){
            CNode *newNode = new CNode(currentL2->data(), currentL2->next());
            newNode->setNext(currentL1->next());
            currentL1->setNext(newNode);
            currentL1 = newNode->next();
	...
	{
		int counter = 0;
        CNode *currentL1 = first->next();
        CNode *previous = first;

        while(previous != 0){

            if(currentL1 != 0 && currentL1->data() == previous->data()){
                counter++;
                currentL1 = currentL1->next();
                continue;
            }
            else{
                previous->setNext(currentL1);
                previous = currentL1;

                if(previous != 0){
                    currentL1 = previous->next();
                }
            }
        }
		return counter;
