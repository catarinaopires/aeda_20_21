...
bool PaintCatalogItem::operator<(const PaintCatalogItem& pci) const {
    if(paintItem->year == pci.paintItem->year){
        if(paintItem->author == pci.paintItem->author){
            return paintItem->title < pci.paintItem->title;
        }
        return paintItem->author < pci.paintItem->author;
    }
    return paintItem->year > pci.paintItem->year;
}


bool ExhibitionItem::operator<(const ExhibitionItem& pci) const {
    if(toShow->getYear() == pci.toShow->getYear()){
        return toShow->getPrice() < pci.toShow->getPrice();
    }
    return toShow->getYear() > pci.toShow->getYear();
}
